...
#### User Story 5 - Live Performance Controls (Priority: P1 · Phase 2)
...
#### User Story 6 - Deep Parameter Control via SysEx (Priority: P1 · Phase 3)
...
#### User Story 7 - Reverse Engineering & Memory Persistence (Priority: P2 · Phase 3)
...
- **FR-013**: System MUST send Control Change messages for: Volume (CC 7), Reverb Send Level (CC 91), Chorus Send Level (CC 93), Pan (CC 10), Expression (CC 11).
- **FR-014**: System MUST provide on-screen controls (sliders or knobs) for each Phase 2 parameter with real-time MIDI transmission on value change.

### Functional Requirements — Phase 3 (Deep Hardware / SysEx — Future)

- **FR-015**: System MUST send Universal Realtime SysEx for Master Volume, Master Fine Tuning, and Master Coarse Tuning.
- **FR-016**: System MUST send Global Parameter Control SysEx messages for reverb engine settings (type and time) and chorus engine settings (type, mod rate, mod depth, feedback, send-to-reverb).
- **FR-017**: System SHOULD investigate undocumented MIDI commands via traffic analysis of the official Roland app to discover memory persistence capabilities (Key Touch, tone retention after power-off).
- **FR-018**: System MUST send Control Change messages for sound-shaping parameters: Cutoff (CC 74), Resonance (CC 71), Attack (CC 73), Decay (CC 75), Release (CC 72), Vibrato Rate (CC 76), Vibrato Depth (CC 77), Vibrato Delay (CC 78), Portamento On/Off (CC 65), Portamento Time (CC 5).
- **FR-019**: System MUST clearly indicate when a control is "tone-dependent" (may not produce an audible effect on all sounds).
...
- **Performance State**: The app's internal mirror of what has been sent to the piano. Attributes: active tone, volume level, expression level, pan position, reverb send, chorus send. Expanded in Phase 3 with: sound-shaping parameter values, tuning values, effect engine settings.
...
- **SC-008**: All live performance controls (volume, reverb, chorus, pan, expression) produce audible changes on the piano with no perceptible latency.

#### Phase 3

- **SC-009**: Master Tuning adjustments are audible (e.g., shifting from A4=440Hz to A4=442Hz).
- **SC-010**: Reverse engineering findings are documented regardless of success — either as implemented features or as "hardware-limited" conclusions.
